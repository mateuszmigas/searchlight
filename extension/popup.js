...
For more info, visit https://reactjs.org/link/mock-scheduler`);
          ...
          var fuzzysort3 = {
            single: function(search, target, options) {
              if (!search)
                return null;
              if (!isObj(search))
                search = fuzzysort3.getPreparedSearch(search);
              if (!target)
                return null;
              if (!isObj(target))
                target = fuzzysort3.getPrepared(target);
              ...
              var algorithm = allowTypo ? fuzzysort3.algorithm : fuzzysort3.algorithmNoTypo;
              return algorithm(search, target, search[0]);
            },
            go: function(search, targets, options) {
              if (!search)
                return noResults;
              search = fuzzysort3.prepareSearch(search);
            ...
            prepare: function(target) {
              if (!target)
                return;
              return { target, _targetLowerCodes: fuzzysort3.prepareLowerCodes(target), _nextBeginningIndexes: null, score: null, indexes: null, obj: null };
            },
            prepareSlow: function(target) {
              if (!target)
                return;
              return { target, _targetLowerCodes: fuzzysort3.prepareLowerCodes(target), _nextBeginningIndexes: fuzzysort3.prepareNextBeginningIndexes(target), score: null, indexes: null, obj: null };
            },
            prepareSearch: function(search) {
              if (!search)
                return;
              return fuzzysort3.prepareLowerCodes(search);
            },
            getPrepared: function(target) {
              if (target.length > 999)
                return fuzzysort3.prepare(target);
              var targetPrepared = preparedCache.get(target);
              if (targetPrepared !== void 0)
                return targetPrepared;
              targetPrepared = fuzzysort3.prepare(target);
              preparedCache.set(target, targetPrepared);
              return targetPrepared;
            },
            getPreparedSearch: function(search) {
              if (search.length > 999)
                return fuzzysort3.prepareSearch(search);
              var searchPrepared = preparedSearchCache.get(search);
              if (searchPrepared !== void 0)
                return searchPrepared;
              searchPrepared = fuzzysort3.prepareSearch(search);
            ...
            algorithm: function(searchLowerCodes, prepared, searchLowerCode) {
              ...
              if (nextBeginningIndexes === null)
                nextBeginningIndexes = prepared._nextBeginningIndexes = fuzzysort3.prepareNextBeginningIndexes(prepared.target);
            ...
            prepareNextBeginningIndexes: function(target) {
              var targetLen = target.length;
              var beginningIndexes = fuzzysort3.prepareBeginningIndexes(target);
  ...
  var useScrollListToIndex = (elementRef, index) => {
    import_react2.default.useEffect(() => {
      if (index !== null && elementRef.current) {
        ...
        element.scrollToItem(index, "smart");
      }
    }, [index]);
  };
  var memoizedRow = import_react2.default.memo(function ListRow(props) {
    ...
    return /* @__PURE__ */ import_react2.default.createElement("div", {
      ...
      className: `popup-search-list-item ${index === selectedIndex ? "popup-search-list-item-selected" : ""} popup-search-list-item-${searchItem.item.type === "BOOKMARK" ? "bookmark" : "tab"}`,
  ...
  var fuzzyFilter = (query, items) => {
    const results = import_fuzzysort.default.go(query, items, {
      keys: ["titleSearchData", "urlSearchData"]
    });
    return results.map((r) => ({
      item: r.obj,
      titleHighlight: r[0] ? import_fuzzysort.default.highlight(r[0], "<b>", "</b>") : r.obj.title,
      urlHightlight: r[1] ? import_fuzzysort.default.highlight(r[1], "<b>", "</b>") : r.obj.url
    }));
  };
  ...
  var getAllTabs = () => __async(void 0, null, function* () {
    ...
    return tabs.filter((t) => !!t.title && !!t.url && !!t.id && !t.active).map((t) => ({
      type: "TAB",
      id: t.id,
      title: t.title,
      url: t.url
  ...
  var getAllSearchItems = () => __async(void 0, null, function* () {
    ...
    return tabs.concat(bookmarks).map((i) => __spreadProps(__spreadValues({}, i), {
      titleSearchData: import_fuzzysort2.default.prepare(i.title),
      urlSearchData: import_fuzzysort2.default.prepare(i.url)
    }));
  });
